...
if (!global._pendingRequests) global._pendingRequests = { byModel: {}, byAccount: {}, byClientToken: {} };
// Keep hot-reloaded processes compatible with the pre-client-token shape.
global._pendingRequests.byClientToken ||= {};
...
export function trackPendingRequest(model, provider, connectionId, started, error = false, clientTokenId = null) {
  const modelKey = provider ? `${model} (${provider})` : model;
  const timerKey = `${clientTokenId || "-"}|${connectionId || "-"}|${modelKey}`;
  ...
  // Track client-token traffic separately so two keys bound to one account
  // can show independent realtime status in Usage & Analytics.
  if (clientTokenId) {
    const tokenConnections = pendingRequests.byClientToken[clientTokenId] ||= {};
    const tokenConnectionId = connectionId || "__unknown__";
    const tokenModels = tokenConnections[tokenConnectionId] ||= {};
    tokenModels[modelKey] = Math.max(0, (tokenModels[modelKey] || 0) + (started ? 1 : -1));
    if (tokenModels[modelKey] === 0) delete tokenModels[modelKey];
    if (Object.keys(tokenModels).length === 0) delete tokenConnections[tokenConnectionId];
    if (Object.keys(tokenConnections).length === 0) delete pendingRequests.byClientToken[clientTokenId];
  }
  ...
  if (started) {
    ...
    pendingTimers[timerKey] = setTimeout(() => {
      ...
      if (clientTokenId) {
        const tokenConnections = pendingRequests.byClientToken[clientTokenId];
        const tokenModels = tokenConnections?.[connectionId || "__unknown__"];
        if (tokenModels?.[modelKey] > 0) delete tokenModels[modelKey];
        if (tokenModels && Object.keys(tokenModels).length === 0) delete tokenConnections[connectionId || "__unknown__"];
        if (tokenConnections && Object.keys(tokenConnections).length === 0) delete pendingRequests.byClientToken[clientTokenId];
      }
...
export async function getActiveRequests() {
  ...
  for (const [clientTokenId, connections] of Object.entries(pendingRequests.byClientToken)) {
    for (const [connectionId, models] of Object.entries(connections || {})) {
      for (const [modelKey, count] of Object.entries(models || {})) {
        if (count <= 0) continue;
        const resolvedConnectionId = connectionId === "__unknown__" ? null : connectionId;
        const accountName = resolvedConnectionId
          ? (connectionMap[resolvedConnectionId] || `Account ${resolvedConnectionId.slice(0, 8)}...`)
          : "Unknown account";
        const match = modelKey.match(/^(.*) \((.*)\)$/);
  ...
  return { activeRequests, activeClientTokens, recentRequests, errorProvider };
}
