...
import { getClientAssignmentsByAccount, getProviderConnections } from "@/lib/localDb";
...
export async function GET(request) {
  try {
    ...
    const assignments = await getClientAssignmentsByAccount(pageConnections.map((connection) => connection.id));
    for (const connection of pageConnections) {
      connection.assignedClients = assignments[connection.id] || [];
    }
