...
async function requireValidApiKey(request) {
  ...
  if (authentication.error) {
    return { error: codedErrorResponse(authentication.error.status, authentication.error.code, authentication.error.message), client: null };
  }
...
export async function handleVideoCreate(request, action) {
  const authentication = await requireValidApiKey(request);
  ...
  while (true) {
    ...
    if (!credentials || credentials.allRateLimited) {
      ...
      if (excludeConnectionIds.size === 0) {
        if (authentication.client) return codedErrorResponse(503, "NO_AVAILABLE_ACCOUNT", `No credentials for provider: ${provider}`);
    ...
    if (shouldFallback && !credentials.routingBound && CREATE_ROTATION_STATUSES.has(result.status)) {
