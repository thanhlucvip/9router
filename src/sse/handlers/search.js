...
export async function handleSearch(request) {
  ...
  if (comboModels) {
    ...
    return handleComboChat({
      body,
      models: comboModels,
      handleSingleModel: (b, m) => handleSingleProviderSearch(b, m, request, apiKey, settings, authenticatedClient),
      log,
  ...
  return handleSingleProviderSearch(body, providerInput, request, apiKey, settings, authenticatedClient);
}

async function handleSingleProviderSearch(body, providerInput, request, apiKey, settings, authenticatedClient = null) {
  const query = body.query;
  ...
  while (true) {
    ...
    let credentials = await getProviderCredentials(providerId, excludeConnectionIds, searchLockKey, { authenticatedClient });
    ...
    if (!credentials && fallbackProviderId) {
      credentials = await getProviderCredentials(fallbackProviderId, excludeConnectionIds, searchLockKey, { authenticatedClient });
      if (credentials?.boundError) {
        return codedErrorResponse(credentials.boundError.status, credentials.boundError.code, credentials.boundError.message);
      }
      if (credentials) {
    ...
    if (!credentials || credentials.allRateLimited) {
      ...
      if (excludeConnectionIds.size === 0) {
        ...
        if (authenticatedClient) return codedErrorResponse(503, "NO_AVAILABLE_ACCOUNT", `No credentials for provider: ${providerId}`);
    ...
    try {
      refreshedCredentials = await checkAndRefreshToken(credentialProviderId, credentials);
    } catch (error) {
      if (credentials.routingBound) {
        return codedErrorResponse(502, "BOUND_ACCOUNT_AUTH_ERROR", error.message || "Authentication failed for the account assigned to this client token.");
      }
      throw error;
    }
    ...
    if (boundFailure) return codedErrorResponse(boundFailure.status, boundFailure.code, boundFailure.message);
