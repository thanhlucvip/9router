...
export async function handleFetch(request) {
  ...
  if (comboModels) {
    ...
    return handleComboChat({
      body,
      models: comboModels,
      handleSingleModel: (b, m) => handleSingleProviderFetch(b, m, request, apiKey, settings, authenticatedClient),
      log,
  ...
  return handleSingleProviderFetch(body, providerInput, request, apiKey, settings, authenticatedClient);
}

async function handleSingleProviderFetch(body, providerInput, request, apiKey, settings, authenticatedClient = null) {
  const targetUrl = body.url;
  const format = body.format;
  const maxCharacters = body.max_characters;
  ...
  while (true) {
    const credentials = await getProviderCredentials(providerId, excludeConnectionIds, fetchLockKey, { authenticatedClient });
